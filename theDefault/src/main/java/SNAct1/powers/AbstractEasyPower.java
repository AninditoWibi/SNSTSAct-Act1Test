...
public abstract class AbstractEasyPower extends TwoAmountPower {
    public AbstractEasyPower(String NAME, String ID, PowerType powerType, boolean isTurnBased, AbstractCreature owner, int amount) {
        ...
        Texture normalTexture = TexLoader.getTexture(SNAct1Mod.makePowerPath(ID.replace(getModID() + ":", "") + "32.png"));
        Texture hiDefImage = TexLoader.getTexture(SNAct1Mod.makePowerPath(ID.replace(getModID() + ":", "") + "84.png"));
        if (hiDefImage != null) {
